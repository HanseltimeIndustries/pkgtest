 ...
 */
export function isWindowsProblem(_pkgManager: StandardizedTestConfigEntry['packageManagers'][0]) {
    if (process.platform !== "win32") {
        return false;
    }
    const { packageManager: pkgManager, options: pkgManagerOptions, alias: pkgManagerAlias } = _pkgManager
    ...
    if (pkgManager === PkgManager.YarnBerry) {
        const nodeLinker = (pkgManagerOptions as YarnV4Options | undefined)?.yarnrc?.nodeLinker;
        if (!nodeLinker || nodeLinker === 'pnp' && lt(process.version, '20.x')) {
            return true;
        }
    }
    return false;
