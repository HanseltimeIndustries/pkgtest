...
export class TestRunner {
    ...
    constructor(options: {
        ...
        Object.assign(this, options as TestRunner)
    }
    ...
    async runTests(options: {
        ...
        console.log(`Test Suite for Module ${this.modType}, Package Manager ${this.pkgManager}, Run with ${this.runBy}`)
        ...
        for (const testFile of this.testFiles) {
            try {
                ...
                if (testNames.length > 0) {
                    if (!micromatch.isMatch(testFile.replace(this.projectDir, ''), testNames)) {
                    ...
                    exec(cmd, {
                        ...
                        if (!err) {
                            if (testTimeMs >= timeout) {
                                console.error(`${chalk.red('Test exceeded timeout')}: ${timeout} ms`)
                            } 
