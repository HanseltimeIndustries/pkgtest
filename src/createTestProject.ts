 ...
 * @returns 
 */
export async function createTestProject(context: {
    projectDir: string
    ...
    const packageJson = JSON.parse((await readFile(join(projectDir, 'package.json'))).toString())
    ...
    await writeFile(join(testProjectDir, 'package.json'), JSON.stringify(pkgJson, null, 4))
    if (debug) {
    ...
    switch(pkgManager) {
        ...
        break;
        default:
            throw new Error(`Unimplemented package manager install for: ${pkgManager}`)
    }
    if (debug) {
    ...
    const copiedTestFiles = await Promise.all(testFiles.map(async (tf) => {
    ...
    if (typescript) {
        ...
        await writeFile(join(testProjectDir, configFilePath), JSON.stringify(tsConfig, null, 4))
        if (debug) {
        ...
        runBy.forEach((rBy) => {
            ...
            switch(rBy) {
                ...
                case RunBy.TsNode:
                case RunBy.Tsx:
                    testFiles = copiedTestFiles.map((srcFile) => srcFile.replace(absSrcPath, absBuildPath))
                break;
                default:
                    throw new Error(`Unimplemented testFile mapping for ${rBy} and typescript configuration!`)
            }
            runners.push(new TestRunner({
        ...
        runBy.forEach((rBy) => {
            ...
            switch(rBy) {
                ...
                break;
                default:
                    throw new Error(`Unimplemented testFile mapping for ${rBy} and non-typescript configuration!`)
            }
            runners.push(new TestRunner({
                projectDir: testProjectDir,
                binRunCommand: binRunCmd,
