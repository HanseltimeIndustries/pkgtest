...
export async function getConfig(configFile?: string) {
    ...
    if (configFile) {
        ...
        const potentialFilePaths = [ 'json', 'js', 'cjs', 'mjs', 'ts' ].map((ext) => resolve(process.cwd(), `${DEFAULT_CONFIG_FILE_NAME_BASE}.${ext}`))
        ...
        if (multiMatch.length > 0) {
            throw new Error(`unable to determing ${LIBRARY_NAME} config file!  Found multiple matches:\n${
                [
                    resolvedFile,
                    ...multiMatch
                ].join('\n')
        ...
        if (!resolvedFile) {
            throw new Error(`Unable to find ${LIBRARY_NAME} config file in default locations:\n${
    ...
    if (!existsSync(resolvedFile)) {
        throw new Error(`Could not find a ${LIBRARY_NAME} config file ${resolvedFile}`)
    }
    ...
    throw new Error(`Unimplented handling of file extension for config file ${resolvedFile}`)
}
